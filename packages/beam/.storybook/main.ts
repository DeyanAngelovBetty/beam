...
const beamLabSrc = fileURLToPath(new URL('../../beam-lab/src/index.ts', import.meta.url));

const config: StorybookConfig = {
  framework: '@storybook/react-vite',
  stories: [
    '../src/**/*.stories.@(ts|tsx)',
    '../../beam-lab/src/**/*.stories.@(ts|tsx)', // BeamLab/ resident (the graduated Theme Lab)
  ...
  viteFinal: async (viteConfig) => {
    viteConfig.resolve ??= {};
    viteConfig.resolve.alias = { ...viteConfig.resolve.alias, '@betty/beam': beamSrc, '@betty/beam-lab': beamLabSrc };
    return viteConfig;
  },
};
