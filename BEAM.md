...
| `product/title/{tint,underlineWeight,underlineFade}` | `titleSeeds` → the gradient title recipe in `BeamPageHeader` (`--beam-title-tint` per-mode; `--beam-title-underline-{weight,fade}` per-product). Default on; dial `tint`→0% / `underlineWeight`→0px to disable *(2026-08-11; light-mode tint values are starting proposals — Deyan tunes)* |
