...
- **Header expand-ALL caret — CANDIDATE DESIGN FEEDBACK for official (2b, 2026-09-21).** Official's `TableHeaderActions` renders an expand-all caret in the header rail whenever row expansion is enabled. **Estate design ruling: this is a UX smell** (a header affordance that expands every row at once is rarely wanted and clutters the rail), so it is **banned estate-wide** — every Beam page passes `expandAll={false}`. The prop defaults to `true` only to preserve official-subset fidelity. **Flag for a menu/rail-touching upstream exchange: propose official make header expand-all opt-IN, or drop it.** (Not a lane capability — a subtraction from official's default.)
- **Row-state washes use CSS vars, not theme literals — CANDIDATE BUG-FIX for official (2026-09-24).** Official's `Table/Table.styles.ts` `body` paints row hover/selected from `theme.palette.action.hover` and `alpha(theme.palette.primary.main, …)` — build-time literals resolved once from the DEFAULT color scheme. Under MUI `cssVariables` + a non-default scheme (our `data-beam-mode="dark"`) they don't track the mode, so the wash bakes the light value and goes invisible on a dark surface (the row splits — the var-based rail lit, the literal data cells didn't). **Purity divergence: we edited the ported file's official code (not a marked LANE addition)** — our `body` now paints from CSS vars (`var(--mui-palette-action-hover)`, `rgba(var(--mui-palette-primary-mainChannel) / var(--mui-palette-action-selectedOpacity))`), matching both MUI's own built-in `.Mui-selected`/`hover` (which official gets right, via `theme.vars`) and our rail wash. **Flag for a Table-touching upstream exchange: official's literal washes are latently broken under ANY non-default scheme — propose they read `theme.vars`.** (Regression fix 2026-09-23; BEAM.md §6.12.)
...
5. ~~**FilterBar paradigm:** rewrite to official's `definitions` + `useTableFilters` (+ their URL sync), or keep composition-by-children + page-owned draft/applied?~~ **DECIDED — Wave 1 (2026-09-17): rewrote to official's `definitions` + `useTableFilters`; the composition-by-children bar and page-owned draft/applied are deleted (zero split-brain).** Flagged deviations: react-router `useSearchParams` URL sync (official's window.history is dead under HashRouter), native `datetime-local` for `dateTime`, local `TablePagination*` until Wave 2, and a local `clearValue(key)` controller method for the orphan rule. Product affordances official can't express stay page-level composition around the unmodified bar. **UPSTREAM PITCH DECLINED (2026-09-21):** official `TableFilters` stays synced to backend filter capabilities *by design* — it does not grow generic money/date/multiSelect control types or dynamic/addable fields. So these are now **STANDING lane patterns, resolved-as-declined — permanent, not temporaries with an exit condition:** (a) Gaspar Transactions' page-side **dynamic/addable-filters (`[+]`) composition** stays as-is; (b) the **money / date / multiSelect** control-type gaps stay page-side compositions (money → two text defs + lenient parse; date → `dateTime` sliced; multiSelect → page-composed control on the same controller). **Revisit only if backend capabilities grow to include dynamic/multi-type filtering.**
